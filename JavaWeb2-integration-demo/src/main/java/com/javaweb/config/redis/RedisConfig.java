...
public class RedisConfig {
	...
	@Autowired
	private RedisParameter2 redisParameter2;
	...
	public RedisConnectionFactory getRedisConnectionFactory2() {
		RedisStandaloneConfiguration redisStandaloneConfiguration = new RedisStandaloneConfiguration();
		redisStandaloneConfiguration.setHostName(redisParameter2.getHost());
		redisStandaloneConfiguration.setPassword(redisParameter2.getPassword());
		redisStandaloneConfiguration.setPort(redisParameter2.getPort());
		GenericObjectPoolConfig<?> genericObjectPoolConfig = new GenericObjectPoolConfig<>();
		genericObjectPoolConfig.setMinIdle(redisParameter2.getMinIdle());
		genericObjectPoolConfig.setMaxIdle(redisParameter2.getMaxIdle());
		genericObjectPoolConfig.setMaxTotal(redisParameter2.getMaxTotal());
		genericObjectPoolConfig.setMaxWaitMillis(redisParameter2.getMaxWaitMillis());
		LettuceClientConfiguration lettuceClientConfiguration = LettucePoolingClientConfiguration.builder().commandTimeout(Duration.ofMillis(redisParameter2.getCommandTimeout())).poolConfig(genericObjectPoolConfig).build();
        LettuceConnectionFactory factory = new LettuceConnectionFactory(redisStandaloneConfiguration,lettuceClientConfiguration);
        factory.afterPropertiesSet();//必须初始化实例
        return factory;
	}
	...
	public StringRedisTemplate redisTemplate2(){
		StringRedisTemplate stringRedisTemplate = new StringRedisTemplate(getRedisConnectionFactory2());
		Jackson2JsonRedisSerializer<Object> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<Object>(Object.class);
		ObjectMapper objectMapper = new ObjectMapper();
		objectMapper.setVisibility(PropertyAccessor.ALL,Visibility.ANY);
		objectMapper.activateDefaultTyping(LaissezFaireSubTypeValidator.instance,ObjectMapper.DefaultTyping.NON_FINAL);
